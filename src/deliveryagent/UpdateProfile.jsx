...
export default function UpdateProfile() 
{
  ...
  useEffect(() => {
    const storedCustomer = sessionStorage.getItem('agent');
    if (storedAgent) {
      setFormData(JSON.parse(storedAgent));
    }
  }, []);
  ...
  const handleSubmit = async (e) => 
  {
    ...
    try 
    {
      const response = await axios.put(`${config.url}/agent/updateprofile`, formData);
      if (response.status === 200) 
        {
        setMessage(response.data);
        setError('');
        sessionStorage.setItem('agent', JSON.stringify(formData)); // Update local session data
      }
    } 
    catch (error) 
    {
